...
describe("Calendar", () => {
  ...
  describe("Basics", () => {
    ...
    it("should constrain the visible region depending on the minValue", () => {
      const { getAllByRole, getByLabelText } = render(
        <Calendar minValue={new CalendarDate(2024, 2, 10)} value={new CalendarDate(2024, 2, 14)} visibleMonths={3} />
      );
  ...
  describe("Keyboard interactions", () => {
    it("should select a date on keyDown Enter/Space (uncontrolled)", () => {
      ...
      const wrapper = render(<Calendar autoFocus defaultValue={new CalendarDate(2024, 3, 31)} onChange={onChange} />);
    ...
    it("should not select a date on keyDown Enter/Space if isReadOnly", () => {
      ...
      const wrapper = render(
        <Calendar autoFocus isReadOnly defaultValue={new CalendarDate(2024, 3, 31)} onChange={onChange} />
      );
    ...
    it("should not show error message without isInvalid", () => {
      let { getByRole } = render(
        <Calendar defaultValue={new CalendarDate(2022, 3, 11)} errorMessage="This is a custom error message" />
      );
    ...
    it("should automatically marks selection as invalid using isDateUnavailable", () => {
      function Example() {
        ...
        return (
          <Calendar defaultValue={new CalendarDate(2022, 3, 5)} isDateUnavailable={(date) => isWeekend(date, locale)} />
        );
      }
