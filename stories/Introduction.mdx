...
<div className="sb-container">
  <div className='sb-section-title'>
    ...
    <p style={{ marginBottom: '-15px' }}>
      参考示例项目 <a href="https://gitlab.itcjf.com/fex/resource/biz-starter-react-web/tree/feat.heroui" target="_blank">biz-starter-react-web</a>。
      或者手动配置您的项目，以便可以通过 `shadcn CLI` 添加组件，你的项目至少需要包含以下内容：
    </p>
  </div>
  ...
  <Source
    ...
    code={
      `{
        ...
        "iconLibrary": "lucide",
        "registries": {
          "@tcsk": "https://feat-heroui--68fb36a8575355ea03c9dd6a.chromatic.com/r/{name}.json"
        }
      }`
    }
  />
...
<div className="sb-container">
  <div className='sb-section-title'>
    # 初始化 & 安装组件
    <p style={{ marginBottom: '-15px' }}>
      使用 `shadcn CLI` 的 `search` 命令来搜索组件，`add` 命令来添加组件。
    </p>
  </div>
  <Source language="bash" dark={true} code={
    `
    npx shadcn@latest add @tcsk/core

    npx shadcn@latest add @tcsk/button
    npx shadcn@latest search @tcsk
    `
  } />
</div>
...
<div className="sb-container">
  ...
  <p style={{ marginBottom: '-20px' }}>`.vscode/settings.json`</p>
  <Source language="json" dark={true} code={
    `
      {
        "files.associations": {
